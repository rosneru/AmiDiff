 ...
 */
Diff::Diff(Operation _operation, const SimpleString &_text) :
...
SimpleString Diff::strOperation(Operation op) {
  switch (op) {
    case INSERT:
      return "INSERT";
    case DELETE:
 ...
 * @return text version
 */
SimpleString Diff::toString() const {
  SimpleString prettyText = text;
  // Replace linebreaks with Pilcrow signs.
  prettyText.replace('\n', L'\u00b6');
  return SimpleString("Diff(") + strOperation(operation) + SimpleString(",\"")
      + prettyText + SimpleString("\")");
}
 ...
 */
SimpleString Patch::toString() {
  SimpleString coords1, coords2;
  if (length1 == 0) {
    coords1 = SimpleString::number(start1) + SimpleString(",0");
  } else if (length1 == 1) {
    coords1 = SimpleString::number(start1 + 1);
  } else {
    coords1 = SimpleString::number(start1 + 1) + SimpleString(",")
        + SimpleString::number(length1);
  }
  if (length2 == 0) {
    coords2 = SimpleString::number(start2) + SimpleString(",0");
  } else if (length2 == 1) {
    coords2 = SimpleString::number(start2 + 1);
  } else {
    coords2 = SimpleString::number(start2 + 1) + SimpleString(",")
        + SimpleString::number(length2);
  }
  SimpleString text;
  text = SimpleString("@@ -") + coords1 + SimpleString(" +") + coords2
      + SimpleString(" @@\n");
  ...
  foreach (Diff aDiff, diffs) {
    ...
    text += SimpleString(QUrl::toPercentEncoding(aDiff.text, " !~*'();/?:@&=+$,#"))
        + SimpleString("\n");
  }
...
QList<Diff> diff_match_patch::diff_main(const SimpleString &text1,
                                        const SimpleString &text2) {
  return diff_main(text1, text2, true);
}

QList<Diff> diff_match_patch::diff_main(const SimpleString &text1,
    const SimpleString &text2, bool checklines) {
...
QList<Diff> diff_match_patch::diff_main(const SimpleString &text1,
    const SimpleString &text2, bool checklines, clock_t deadline) {
  ...
  const SimpleString &commonprefix = text1.left(commonlength);
  SimpleString textChopped1 = text1.mid(commonlength);
  SimpleString textChopped2 = text2.mid(commonlength);

  // Trim off common suffix (speedup).
  commonlength = diff_commonSuffix(textChopped1, textChopped2);
  const SimpleString &commonsuffix = textChopped1.right(commonlength);
...
QList<Diff> diff_match_patch::diff_compute(SimpleString text1, SimpleString text2,
  ...
  {
    const SimpleString longtext = text1.length() > text2.length() ? text1 : text2;
    const SimpleString shorttext = text1.length() > text2.length() ? text2 : text1;
  ...
  const SimpleStringList hm = diff_halfMatch(text1, text2);
  if (hm.count() > 0) {
    // A half-match was found, sort out the return data.
    const SimpleString text1_a = hm[0];
    const SimpleString text1_b = hm[1];
    const SimpleString text2_a = hm[2];
    const SimpleString text2_b = hm[3];
    const SimpleString mid_common = hm[4];
...
QList<Diff> diff_match_patch::diff_lineMode(SimpleString text1, SimpleString text2,
    clock_t deadline) {
  // Scan the text on a line-by-line basis first.
  const QList<QVariant> b = diff_linesToChars(text1, text2);
  text1 = b[0].toString();
  text2 = b[1].toString();
  SimpleStringList linearray = b[2].toStringList();
  ...
  int count_delete = 0;
  int count_insert = 0;
  SimpleString text_delete = "";
  SimpleString text_insert = "";
...
QList<Diff> diff_match_patch::diff_bisect(const SimpleString &text1,
    const SimpleString &text2, clock_t deadline) {
...
QList<Diff> diff_match_patch::diff_bisectSplit(const SimpleString &text1,
    const SimpleString &text2, int x, int y, clock_t deadline) {
  SimpleString text1a = text1.left(x);
  SimpleString text2a = text2.left(y);
  SimpleString text1b = safeMid(text1, x);
  SimpleString text2b = safeMid(text2, y);
...
QList<QVariant> diff_match_patch::diff_linesToChars(const SimpleString &text1,
                                                    const SimpleString &text2) {
  SimpleStringList lineArray;
  QMap<SimpleString, int> lineHash;
  ...
  const SimpleString chars1 = diff_linesToCharsMunge(text1, lineArray, lineHash);
  const SimpleString chars2 = diff_linesToCharsMunge(text2, lineArray, lineHash);
...
SimpleString diff_match_patch::diff_linesToCharsMunge(const SimpleString &text,
                                                 SimpleStringList &lineArray,
                                                 QMap<SimpleString, int> &lineHash) {
  int lineStart = 0;
  int lineEnd = -1;
...
void diff_match_patch::diff_charsToLines(QList<Diff> &diffs,
                                         const SimpleStringList &lineArray) {
...
int diff_match_patch::diff_commonPrefix(const SimpleString &text1,
                                        const SimpleString &text2) {
...
int diff_match_patch::diff_commonSuffix(const SimpleString &text1,
                                        const SimpleString &text2) {
  // Performance analysis: http://neil.fraser.name/news/2007/10/09/
  const int text1_length = text1.length();
  const int text2_length = text2.length();
...
int diff_match_patch::diff_commonOverlap(const SimpleString &text1,
                                         const SimpleString &text2) {
  // Cache the text lengths to prevent multiple calls.
  const int text1_length = text1.length();
  const int text2_length = text2.length();
  ...
  SimpleString text1_trunc = text1;
  SimpleString text2_trunc = text2;
  ...
  int length = 1;
  while (true) {
    SimpleString pattern = text1_trunc.right(length);
...
SimpleStringList diff_match_patch::diff_halfMatch(const SimpleString &text1,
                                             const SimpleString &text2) {
  if (Diff_Timeout <= 0) {
  ...
  if (longtext.length() < 4 || shorttext.length() * 2 < longtext.length()) {
    return SimpleStringList();  // Pointless.
  }

  // First check if the second quarter is the seed for a half-match.
  const SimpleStringList hm1 = diff_halfMatchI(longtext, shorttext,
      (longtext.length() + 3) / 4);
  // Check again based on the third quarter.
  const SimpleStringList hm2 = diff_halfMatchI(longtext, shorttext,
      (longtext.length() + 1) / 2);
  SimpleStringList hm;
...
SimpleStringList diff_match_patch::diff_halfMatchI(const SimpleString &longtext,
                                              const SimpleString &shorttext,
                                              int i) {
  // Start with a 1/4 length substring at position i as a seed.
  const SimpleString seed = safeMid(longtext, i, longtext.length() / 4);
  int j = -1;
  SimpleString best_common;
  SimpleString best_longtext_a, best_longtext_b;
  SimpleString best_shorttext_a, best_shorttext_b;
...
void diff_match_patch::diff_cleanupSemantic(QList<Diff> &diffs) {
  ...
  SimpleString lastequality;  // Always equal to equalities.lastElement().text
  QMutableListIterator<Diff> pointer(diffs);
  ...
  while (thisDiff != NULL) {
    if (prevDiff->operation == DELETE &&
      ...
      SimpleString deletion = prevDiff->text;
      SimpleString insertion = thisDiff->text;
...
void diff_match_patch::diff_cleanupSemanticLossless(QList<Diff> &diffs) {
  SimpleString equality1, edit, equality2;
  SimpleString commonString;
  int commonOffset;
  int score, bestScore;
  SimpleString bestEquality1, bestEdit, bestEquality2;
...
int diff_match_patch::diff_cleanupSemanticScore(const SimpleString &one,
                                                const SimpleString &two) {
...
SimpleString diff_match_patch::diff_prettyHtml(const QList<Diff> &diffs) {
  SimpleString html;
  SimpleString text;
  foreach(Diff aDiff, diffs) {
    text = aDiff.text;
    ...
    switch (aDiff.operation) {
      case INSERT:
        html += SimpleString("<ins style=\"background:#e6ffe6;\">") + text
            + SimpleString("</ins>");
        break;
      case DELETE:
        html += SimpleString("<del style=\"background:#ffe6e6;\">") + text
            + SimpleString("</del>");
        break;
      case EQUAL:
        html += SimpleString("<span>") + text + SimpleString("</span>");
        break;
    }
  }
  return html;
...
SimpleString diff_match_patch::diff_text1(const QList<Diff> &diffs) {
  SimpleString text;
  foreach(Diff aDiff, diffs) {
    if (aDiff.operation != INSERT) {
      text += aDiff.text;
...
SimpleString diff_match_patch::diff_text2(const QList<Diff> &diffs) {
  SimpleString text;
  foreach(Diff aDiff, diffs) {
    if (aDiff.operation != DELETE) {
      text += aDiff.text;
...
SimpleString diff_match_patch::diff_toDelta(const QList<Diff> &diffs) {
  SimpleString text;
  foreach(Diff aDiff, diffs) {
    switch (aDiff.operation) {
      case INSERT: {
        SimpleString encoded = SimpleString(QUrl::toPercentEncoding(aDiff.text,
            " !~*'();/?:@&=+$,#"));
        text += SimpleString("+") + encoded + SimpleString("\t");
        break;
      }
      case DELETE:
        text += SimpleString("-") + SimpleString::number(aDiff.text.length())
            + SimpleString("\t");
        break;
      case EQUAL:
        text += SimpleString("=") + SimpleString::number(aDiff.text.length())
...
QList<Diff> diff_match_patch::diff_fromDelta(const SimpleString &text1,
                                             const SimpleString &delta) {
  QList<Diff> diffs;
  int pointer = 0;  // Cursor in text1
  SimpleStringList tokens = delta.split("\t");
  foreach(SimpleString token, tokens) {
    if (token.isEmpty()) {
    ...
    SimpleString param = safeMid(token, 1);
    switch (token[0].toLatin1()) {
      ...
      case '=': {
        ...
        n = param.toInt();
        if (n < 0) {
          throw SimpleString("Negative number in diff_fromDelta: %1").arg(param);
        }
        SimpleString text;
      ...
      default:
        throw SimpleString("Invalid diff operation in diff_fromDelta: %1")
            .arg(token[0]);
    }
  }
  if (pointer != text1.length()) {
    throw SimpleString("Delta length (%1) smaller than source text length (%2)")
...
int diff_match_patch::match_main(const SimpleString &text, const SimpleString &pattern,
                                 int loc) {
  // Check for null inputs.
...
int diff_match_patch::match_bitap(const SimpleString &text, const SimpleString &pattern,
                                  int loc) {
...
QMap<QChar, int> diff_match_patch::match_alphabet(const SimpleString &pattern) {
  QMap<QChar, int> s;
  int i;
...
void diff_match_patch::patch_addContext(Patch &patch, const SimpleString &text) {
  if (text.isEmpty()) {
    return;
  }
  SimpleString pattern = safeMid(text, patch.start2, patch.length1);
  int padding = 0;
  ...
  // Add the prefix.
  SimpleString prefix = safeMid(text, std::max(0, patch.start2 - padding),
      patch.start2 - std::max(0, patch.start2 - padding));
  if (!prefix.isEmpty()) {
    patch.diffs.prepend(Diff(EQUAL, prefix));
  }
  // Add the suffix.
  SimpleString suffix = safeMid(text, patch.start2 + patch.length1,
      std::min(text.length(), patch.start2 + patch.length1 + padding)
...
QList<Patch> diff_match_patch::patch_make(const SimpleString &text1,
                                          const SimpleString &text2) {
  // Check for null inputs.
  if (text1.isNull() || text2.isNull()) {
...
QList<Patch> diff_match_patch::patch_make(const QList<Diff> &diffs) {
  ...
  const SimpleString text1 = diff_text1(diffs);
  return patch_make(text1, diffs);
}
...
QList<Patch> diff_match_patch::patch_make(const SimpleString &text1,
  ...
  // context info.
  SimpleString prepatch_text = text1;
  SimpleString postpatch_text = text1;
  foreach(Diff aDiff, diffs) {
...
QPair<SimpleString, QVector<bool> > diff_match_patch::patch_apply(
    QList<Patch> &patches, const SimpleString &sourceText) {
  SimpleString text = sourceText;  // Copy to preserve original.
  if (patches.isEmpty()) {
    return QPair<SimpleString,QVector<bool> >(text, QVector<bool>(0));
  }

  // Deep copy the patches so that no changes are made to originals.
  QList<Patch> patchesCopy = patch_deepCopy(patches);

  SimpleString nullPadding = patch_addPadding(patchesCopy);
  ...
  foreach(Patch aPatch, patchesCopy) {
    ...
    SimpleString text1 = diff_text1(aPatch.diffs);
    int start_loc;
    int end_loc = -1;
  ...
  return QPair<SimpleString, QVector<bool> >(text, results);
}


SimpleString diff_match_patch::patch_addPadding(QList<Patch> &patches) {
  short paddingLength = Patch_Margin;
  SimpleString nullPadding = "";
...
void diff_match_patch::patch_splitMax(QList<Patch> &patches) {
  ...
  SimpleString precontext, postcontext;
  Patch patch;
  int start1, start2;
  bool empty;
  Operation diff_type;
  SimpleString diff_text;
  QMutableListIterator<Patch> pointer(patches);
  Patch bigpatch;
...
SimpleString diff_match_patch::patch_toText(const QList<Patch> &patches) {
  SimpleString text;
  foreach(Patch aPatch, patches) {
...
QList<Patch> diff_match_patch::patch_fromText(const SimpleString &textline) {
  QList<Patch> patches;
  if (textline.isEmpty()) {
    return patches;
  }
  SimpleStringList text = textline.split("\n", SimpleString::SkipEmptyParts);
  Patch patch;
  ...
  while (!text.isEmpty()) {
    if (!patchHeader.exactMatch(text.front())) {
      throw SimpleString("Invalid patch string: %1").arg(text.front());
    }
    ...
    while (!text.isEmpty()) {
      ...
      if (sign == '-') {
        ...
        // WTF?
        throw SimpleString("Invalid patch mode '%1' in: %2").arg(sign).arg(line);
        return QList<Patch>();
      }
      text.removeFirst();
    }
